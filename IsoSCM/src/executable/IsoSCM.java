...
public class IsoSCM {
	...
	public static void main(String[] args) throws IOException, JAXBException {
		String jarName = new File(IsoSCM.class.getProtectionDomain().getCodeSource().getLocation().getPath()).getName();
		
		
		{
			SimpleDateFormat format = new SimpleDateFormat("yyMMddHHmmssSSS");
			ThreadContext.put("log_date", format.format(new Date()));
			
			InputStream versionStream = IsoSCM.class.getClassLoader().getResourceAsStream("version.txt");
			Scanner s = new Scanner(versionStream);
			String version = s.nextLine();
			logger.info("IsoSCM "+version);
			List<String> jvmArgs = ManagementFactory.getRuntimeMXBean().getInputArguments();
			String command = String.format("java -jar %s %s %s", StringUtils.join(jvmArgs," "), jarName, StringUtils.join(args," "));
		...
		try{
			...
			if(jc.getParsedCommand()==null || jc.getParsedCommand().equals("-h")){
				...
				//pre-process
				{
					...
					SamReader sfr = SamReaderFactory.makeDefault().open(bamFile);
